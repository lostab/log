	...
	def get(self):
		...
		if not site:
			...
			if users.is_current_user_admin():
				...
				path = os.path.join(os.path.dirname(__file__), 'templates/newpost.html')
				self.response.out.write(template.render(path, template_values))
			else:
				self.redirect('/')
	def post(self):
			...
			if users.is_current_user_admin() and key:
				try:
					...
					if comment:
						...
						path = os.path.join(os.path.dirname(__file__), 'templates/editcomment.html')
						self.response.out.write(template.render(path, template_values))
					else:
						self.redirect('/')
				except:
		...
		if not site:
			...
			try:
				...
				if outputcaptcha:
					...
					path = os.path.join(os.path.dirname(__file__), 'templates/guestbook.html')
					self.response.out.write(template.render(path, template_values))
				else:
					self.redirect('/')
			except:
